...
├─ public/rhythm.html  # 팡이 리듬 모드 (WebAudio로 곡 생성, 노트와 동일 시계로 싱크)
...
## 팡이 리듬 모드 규칙
- 노래 박자에 맞춰 9칸에서 팡이가 순서대로 등장, 팡이를 감싼 **접근 링**이 줄어듦
- 링이 팡이에 딱 맞는 순간 터치 → **PERFECT**(±0.085s) / 살짝 어긋나면 **GOOD**(±0.18s) / 놓치면 **MISS**
- **체력 개념 없음** — 미스해도 곡은 끝까지 진행
- 한 칸에 팡이가 여러 마리면 나온 수만큼 **연달아 터치**
- 콤보 보너스 + 정확도 등급(S/A/B/C/D), 최종 점수가 리듬 랭킹에 반영
- 곡/난이도는 `public/rhythm.html`의 `CHART`(박자별 노트)와 `BPM`으로 편집. 실제 음원을 쓰려면 `<audio>` 추가 후 `songStart`를 오디오 재생 시각에 맞추면 됩니다.
