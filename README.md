...
별도 로그인·닉네임 입력 없이 접속하면 바로 플레이 방법 화면이 뜨고, 손님 닉네임(예: 손님1234)이 자동으로 부여됩니다. 랭킹은 브라우저별로 저장됩니다.
...
├─ public/songs/        # 실제 음원(mp3) 1곡 + 추가 방법 README (mp3는 git 제외)
...
## 게임 규칙
- **스윙 버튼** 한 곡으로 리듬 패턴이 고정되어 바로 플레이
- 노래 박자에 맞춰 4칸에서 팡이가 등장, 칸 위에 **남은 시간(3→2→1)**이 숫자로 표시됨
- 3초 안에 터치(또는 키보드 **E**(좌상단)·**O**(우상단)·**F**(좌하단)·**K**(우하단)) → **GOOD** / 시간 안에 못 누르면 **MISS**
- **체력 개념 없음** — 미스해도 곡은 끝까지 진행
- 한 칸에 팡이가 여러 마리면 그 수만큼 연달아 탁-탁-탁 눌러야 GOOD
- 정확도 등급(S/A/B/C/D), 최종 점수가 **랭킹**에 반영
- 게임 중 **⏸ 일시정지**(계속하기/처음부터/나가기) 가능
- 기본은 밝은 테마, 우상단 **🌙 버튼**으로 어두운 테마 전환 가능 (선택은 브라우저에 저장됨)
...
| 곡 | BPM | 리듬 패턴 |
|---|---|---|
| 🎺 스윙 버튼 | 152.0 | `swingEighth` — 1.5박 간격, 가끔 스윙 추가음·연타 (또는 직접 작성한 채보, 아래 참고) |
...
- 랭킹 키는 곡별로 분리됩니다: `rhythm:<곡id>:<diffKey>`
...
## 커스터마이징 포인트
- 반응 제한시간/점수: `public/index.html` 상단 `REACT_WINDOW`, `SCORE_GOOD`
- 곡별 채보 패턴: `chartSwingEighth` (홀 간격, 연타 확률 등)
