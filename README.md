...
## 게임 규칙
- **스윙 버튼** 한 곡, 보통 난이도로 리듬 패턴이 고정되어 바로 플레이
- 노래 박자에 맞춰 4칸에서 팡이가 등장, 팡이를 감싼 **접근 링**이 줄어듦
- 링이 팡이에 딱 맞는 순간 터치(또는 키보드 **E**(좌상단)·**F**(우상단)·**O**(좌하단)·**K**(우하단)) → **PERFECT**(±0.085s) / 살짝 어긋나면 **GOOD**(±0.18s) / 놓치면 **MISS**
- **체력 개념 없음** — 미스해도 곡은 끝까지 진행
- 한 칸에 팡이가 여러 마리면 **링도 마리 수만큼** 차례로 닫힘 → 순서대로 연달아 터치
- ✋ **꾹 누르기(홀드) 노트**: 손을 뗀 채로 나오는 팡이는 링이 다 채워질 때까지 눌러야 함 — 일찍 떼면 MISS
...
| 곡 | 난이도 | BPM | 리듬 패턴 |
|---|---|---|---|
| 🎺 스윙 버튼 | 보통 ★★☆ | 152.0 | `swingEighth` — 1.5박 간격, 가끔 스윙 추가음·연타·홀드 (또는 직접 작성한 채보, 아래 참고) |
