...
곡마다 리듬 패턴(chart 타입)이 곡의 실제 그루브에 맞춰 고정되어 있습니다 — 자세한 생성 로직은 `public/index.html`의 `chartTriplet`/`chartSwingEighth` 참고.
...
- `chart`: `'triplet'`/`'swingEighth'` 중 곡 느낌에 맞는 것을 고르거나, 전용 패턴이 필요하면 `index.html`에 새 `chartXxx()` 함수를 추가해 연결
